...
unsigned char GRandomTextureBuffer[3*1280*720];
GLuint GRandomTexture;
...
void InitGraphics()
{
	...
	//fill the random texture buffer with random data and create a texture from it
	//for(int i = 0; i < sizeof(GRandomTextureBuffer); i++) 
	//	GRandomTextureBuffer[i] = (unsigned char)rand()%256;
	printf("Creating texture\n");
	glGenTextures(1, &GRandomTexture);
	check();
	glBindTexture(GL_TEXTURE_2D, GRandomTexture);
	check();
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1280, 720, 0, GL_RGB, GL_UNSIGNED_BYTE, GRandomTextureBuffer);
	check();
...
{
	int pos = 0;
	while(pos < sizeof(GRandomTextureBuffer))
	{
		int spc = sizeof(GRandomTextureBuffer)-pos;
		int tocopy = sz;
		if(tocopy > spc)
			tocopy = spc;
		memcpy(GRandomTextureBuffer+pos,data,tocopy);
		pos += tocopy;
		break;
	}
}
...
void BeginFrame()
{
	...
	glBindTexture(GL_TEXTURE_2D, GRandomTexture);
	check();
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 1280, 720, GL_RGB, GL_UNSIGNED_BYTE, GRandomTextureBuffer);
	check();
	glBindTexture(GL_TEXTURE_2D, 0);
	check();
